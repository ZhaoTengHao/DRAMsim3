...
uint64_t MemorySystem::GetClockCycle() const { return dram_system_->GetClockCycle(); }
...
bool MemorySystem::AddTransactionID(uint64_t hex_addr, bool is_write, uint64_t id) {
    return dram_system_->AddTransaction(hex_addr, is_write, id);
}
